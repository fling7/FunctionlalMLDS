...
| --- | --- | --- | --- |
| `AgentBody` | Agent / spezialisierte Entity | `Agent`, `Entity.kind = system` | Position, Aktivitaet und Rollenzustand des dynamischen Agenten |
| `TriggerZone` | Szenenbereich | `Entity.kind = environment` | raeumliches Startereignis |
| `SceneBoundary` | Szenengrenze | `Entity.kind = environment` | Gueltigkeit des Szenenbereichs |
| `TargetZone` | Zielbereich | `Entity.kind = asset` | Erreichbarkeit, Belegung und Zielerreichung |
| `ObstacleRegion` | Hindernisbereich | `Entity.kind = environment` | freie, temporaer blockierte oder dauerhaft blockierte Zielregion |
| `SceneStateFlag` | Szenenzustandsmarker | `Entity.kind = environment` | Reaktionsbedarf oder geloester Szenenzustand |
| `ObservationPoint` | Beobachtungspunkt | `Entity.kind = asset` | beobachtbare Verifikation |
| `FeedbackSignal` | Rueckmeldung | `Entity.kind = system` | bestaetigte oder fehlgeschlagene Rueckmeldung |
