...
| A-GAP-06 | Explizite Typisierung und Rolle von `Entity`-Instanzen | `Entity` kann `AgentBody`, `TargetZone`, `ObstacleRegion` oder `FeedbackSignal` aufnehmen, aber das aktuelle Diagramm zeigt kein eigenes `Entity.kind` fuer `agentBody`, `zone`, `boundary`, `asset`, `signal` oder `stateFlag`. | Wird hart, wenn Generatoren oder Pruefer anhand des Modells unterscheiden muessen, welche Entity eine Zone, ein Signal, ein Asset oder ein Zustandstraeger ist. | Typisierung steht als Tabellenkonvention in den Instanzartefakten. | Nein, solange die Konvention diszipliniert verwendet wird. |
...
| A-GAP-08 | Feingranulare Trace-Kante von `Effect` zu konkreter `StateAssertion` | `Effect` beschreibt den versprochenen beobachtbaren Effekt einer `Capability`, und `StateAssertion` beschreibt den erwarteten Zustand eines Schritts. Das aktuelle Metamodell hat keine explizite Kante `Effect -> StateAssertion`. | Wird hart, wenn Nachweiswerkzeuge automatisch pruefen sollen, welche konkrete Zustandsaussage einen Effect realisiert. | Dokumentierter Trace in Tabellen und Validierungsartefakten. | Nein, solange der Trace dokumentarisch akzeptiert ist. |
