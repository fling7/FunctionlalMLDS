...
| `Effect` zu `StateAssertion` | fachlicher Trace in Tabellen | Metamodellkante `Effect -> StateAssertion` existiert nicht; die Rueckbindung ist dokumentarisch. | indirekt | In 5.4 pruefen, ob eine optionale verifies/observes-Kante sinnvoll waere. |
...
| Entity-Typisierung `system`, `environment`, `asset`, `user` | Tabellenkonvention bei Entities | Die Typisierung ist fachlich hilfreich, aber im aktuellen kompakten Klassendiagramm nicht als eigenes Attribut sichtbar. | grenzwertig | In 5.4 pruefen, ob `Entity.kind` in den Kern gehoert. |
