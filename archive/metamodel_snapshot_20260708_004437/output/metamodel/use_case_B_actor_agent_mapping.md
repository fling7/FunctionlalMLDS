...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `ENT-B-01` | `Agent` und damit `Entity` | `ENT-B-01` | `VivianAssistant` | `system` | Modellierte Assistenzinstanz in der virtuellen Szene; fuehrt, bestaetigt, fragt nach, erklaert Fehler und meldet Abschluss fachlich zurueck. | leer in der Baseline | Kein primaerer Actor, keine externe Rolle und keine RuntimeAction. Technische KI-, Sprach-, Avatar- oder Controllerdetails liegen spaeter nur unter `RuntimeBinding -> RuntimeAction`. |
...
| Vivian als Agent/Entity modelliert | `ENT-B-01 VivianAssistant` ist Agent und damit Entity mit `Entity.kind = system`. |
