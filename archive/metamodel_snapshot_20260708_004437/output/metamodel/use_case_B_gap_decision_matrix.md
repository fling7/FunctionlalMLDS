...
| --- | --- | --- |
| `A-GAP-06`: optionale `Entity.kind`-Typisierung | B nutzt `Entity.kind = asset` fuer CoffeeMachine und `Entity.kind = system` fuer Vivian/BrewingRequest bereits als wichtige Konvention. | B bestaetigt eine minimale Kernanpassung `Entity.kind [0..1]`, obwohl sie in B nicht als blockierende harte Luecke gefuehrt wurde. |
