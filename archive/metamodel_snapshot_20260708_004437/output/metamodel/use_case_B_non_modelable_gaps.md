...
| `B-GAP-09` | Formale Trace-Kante von `Effect` zu konkreter `StateAssertion` und zu ValidationOutcome | `Effect` beschreibt versprochene Wirkung, `StateAssertion` beschreibt beobachteten Zustand und `ValidationCase` beschreibt erwartete Ergebnisse. Das Metamodell hat aber keine explizite Kante `Effect -> StateAssertion` oder `ValidationOutcome -> StateAssertion`. | Wird hart, wenn Nachweiswerkzeuge automatisch pruefen sollen, welche konkrete Zustandsaussage einen Effect realisiert und welcher Test diesen Effect validiert. | Dokumentierter Trace in Capability-, RuntimeAction- und ValidationCase-Tabellen. | Nein, solange dokumentarischer Trace akzeptiert wird. |
