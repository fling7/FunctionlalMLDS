...
NODES: dict[str, Node] = {
    ...
    "Condition": Node("Condition", "Condition", 520, 830, 300, 136, "<<Dynamic Functional MLDS>>", ("kind: guard | pre | post | spatial | timing", "expression: String", "randomVariable [0..*]"), "#f3fff8"),
    ...
    "Entity": Node("Entity", "Entity", 80, 1140, 300, 148, "<<Dynamic Functional MLDS>>", ("kind: EntityKind [0..1]", "EntityKind = agent | asset | zone | signal | stateObject", "description: String"), "#f8f5ff"),
...
EDGES: tuple[Edge, ...] = (
    ...
    Edge("UseCase", "ExtensionPoint", "defines\nextension points\n[0..*]", "composition", ((1310, 290), (1460, 290)), (1385, 405)),
    ...
    Edge("Effect", "StateAssertion", "evidenced by\nstate assertion\n[0..*]", "assoc", ((1260, 1424), (1260, 1455), (2500, 1455), (2500, 965), (1250, 965), (1250, 876)), (2220, 950)),
    ...
    Edge("ValidationCase", "RuntimeBinding", "checks runtime binding\n[0..*]", "dependency", ((2295, 1216), (2295, 1250), (1760, 1250), (1760, 1320)), (2030, 1270), dashed=True),
)
...
def render_png(path: Path) -> None:
    try:
        ...
        raise RuntimeError("PNG rendering requires Pillow (PIL).") from exc
    ...
    def centered_text(draw: ImageDraw.ImageDraw, x: float, y: float, text: str, font_obj, fill: str) -> None:
        tw, th = text_size(draw, text, font_obj)
        draw.text((x - tw / 2, y - th / 2), text, font=font_obj, fill=fill)

    def draw_polyline(draw: ImageDraw.ImageDraw, points: tuple[tuple[int, int], ...], fill: str, width: int, dashed: bool) -> None:
        if not dashed:
        ...
        for start, end in zip(points, points[1:]):
            ...
            cursor = 0.0
            while cursor < length:
                segment_end = min(cursor + dash_len, length)
                sx = x1 + dx * (cursor / length)
                sy = y1 + dy * (cursor / length)
                ex = x1 + dx * (segment_end / length)
                ey = y1 + dy * (segment_end / length)
                draw.line(((sx, sy), (ex, ey)), fill=fill, width=width)
                cursor += dash_len + gap_len

    def draw_arrow(draw: ImageDraw.ImageDraw, points: tuple[tuple[int, int], ...], fill: str) -> None:
        if len(points) < 2:
            return
        end = points[-1]
        prev = next((p for p in reversed(points[:-1]) if p != end), points[-2])
        dx = end[0] - prev[0]
        dy = end[1] - prev[1]
        length = max((dx * dx + dy * dy) ** 0.5, 1.0)
        ...
        half = 5
        tip = end
        p1 = (end[0] - ux * size + nx * half, end[1] - uy * size + ny * half)
        p2 = (end[0] - ux * size - nx * half, end[1] - uy * size - ny * half)
        draw.polygon((tip, p1, p2), fill=fill)

    def draw_diamond(draw: ImageDraw.ImageDraw, points: tuple[tuple[int, int], ...], fill: str) -> None:
        if len(points) < 2:
            return
        start = points[0]
        nxt = next((p for p in points[1:] if p != start), points[1])
        dx = nxt[0] - start[0]
        dy = nxt[1] - start[1]
        length = max((dx * dx + dy * dy) ** 0.5, 1.0)
        ux, uy = dx / length, dy / length
        nx, ny = -uy, ux
        center = (start[0] + ux * 8, start[1] + uy * 8)
        half = 6
        diamond = (
            start,
            (center[0] + nx * half, center[1] + ny * half),
            (start[0] + ux * 16, start[1] + uy * 16),
            (center[0] - nx * half, center[1] - ny * half),
        )
        draw.polygon(diamond, fill=fill)

    image = Image.new("RGB", (2600, 1650), "#fbfcfd")
    draw = ImageDraw.Draw(image)
    title_font = font(19, bold=True)
    node_title_font = font(15, bold=True)
    stereo_font = font(12, italic=True)
    attr_font = font(13)
    edge_font = font(13)
    note_font = font(13, bold=True)

    sections = (
        (40, 40, 2520, 430, "A. EAST-ADL Requirements / UseCases: unverfaelschter Kern"),
        (40, 500, 2520, 500, "B. Scenario Layer: geordnete, bedingte und parallele Ablaeufe"),
        (40, 1030, 2520, 570, "C. Functional/Runtime Bridge: Domaenenfaehigkeit vor Technikbindung"),
    )
    draw.text((40, 13), "Kompaktes Metamodell fuer Dynamic Functional MLDS (v0.5)", font=title_font, fill="#243044")
    for x, y, w, h, title in sections:
        draw.rounded_rectangle((x, y, x + w, y + h), radius=6, fill="#ffffff", outline="#d8dee8", width=1)
        draw.text((x + 18, y + 9), title, font=title_font, fill="#243044")

    for edge in EDGES:
        points = edge.points or (anchor(NODES[edge.src], NODES[edge.dst]), anchor(NODES[edge.dst], NODES[edge.src]))
        draw_polyline(draw, points, edge.color, 2 if edge.kind != "inherit" else 1, edge.dashed or edge.kind == "dependency")
        if edge.kind == "composition":
            draw_diamond(draw, points, edge.color)
        draw_arrow(draw, points, edge.color)
        if edge.label:
            x, y = label_position(edge)
            label_lines = edge.label.split("\n")
            line_sizes = [text_size(draw, line, edge_font) for line in label_lines]
            box_w = max(64, max(width for width, _ in line_sizes) + 26)
            box_h = 18 * len(label_lines) + 12
            draw.rounded_rectangle((x - box_w / 2, y - 15, x + box_w / 2, y - 15 + box_h), radius=3, fill="#ffffff", outline="#dbe3ee", width=1)
            for idx, line in enumerate(label_lines):
                centered_text(draw, x, y + idx * 17, line, edge_font, "#243044")

    for node in NODES.values():
        draw.rounded_rectangle((node.x, node.y, node.x + node.w, node.y + node.h), radius=4, fill=node.fill, outline="#927f78", width=2)
        draw.line((node.x, node.y + 44, node.x + node.w, node.y + 44), fill="#927f78", width=1)
        title_y = node.y + 19
        if node.stereo:
            centered_text(draw, node.x + node.w / 2, title_y, node.stereo, stereo_font, "#475569")
            title_y += 17
        centered_text(draw, node.x + node.w / 2, title_y, node.title, node_title_font, "#222b35")
        attr_y = node.y + 55
        for attr in node.attrs:
            for part in wrap(attr, max(20, (node.w - 24) // 7)):
                draw.text((node.x + 14, attr_y), f"+ {part}", font=attr_font, fill="#6b2b18")
                attr_y += 17

    draw.rounded_rectangle((1910, 755, 2465, 827), radius=4, fill="#fff6f6", outline="#df8b8b", width=1)
    draw.text((1928, 770), "Invariant: ScenarioStep hat keine direkte RuntimeAction-Kante.", font=note_font, fill="#8a1f1f")
    draw.text((1928, 795), "Ausfuehrung laeuft nur ueber CapabilityUse -> Capability -> RuntimeBinding.", font=note_font, fill="#8a1f1f")
    image.save(path)
...
def render_svg() -> str:
    ...
    parts = [
        ...
        '<text x="40" y="27" class="section-title">Kompaktes Metamodell für Dynamic Functional MLDS (v0.5)</text>',
    ]
    for x, y, w, h, title in sections:
...
def render_mermaid() -> str:
    return dedent(
        ...
        classDiagram
          ...
          Effect --> "0..*" StateAssertion : evidencedBy
...
def main() -> None:
    ...
    render_png(OUT / "dynamic_functional_mlds_metamodel.png")
