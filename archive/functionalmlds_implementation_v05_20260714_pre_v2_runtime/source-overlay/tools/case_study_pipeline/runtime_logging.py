...
def validate_runtime_event(event: Mapping[str, Any], trace_map: Optional[Mapping[str, Any]] = None) -> List[str]:
    ...
    if event.get("schema_version", RUNTIME_LOG_SCHEMA_VERSION) != RUNTIME_LOG_SCHEMA_VERSION:
    ...
    if event.get("status") not in {"success", "failure", "skipped", "needs_manual_review", None}:
