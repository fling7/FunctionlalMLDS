...
def _recover_project_materialization_if_possible(case_dir: Path, backend_root: Path = DEFAULT_BACKEND_ROOT) -> None:
    ...
    if validation["status"] == "valid":
        update_manifest(
            ...
            output_paths=[project_paths["project_json"], project_paths["room_plan"], project_paths["agents"], project_paths["trace_map"], validation_path],
            metadata={"recovered_without_rerun": True, "metrics": validation.get("metrics", {})},
        )
    ...
    if args.run_llm:
        ...
        for result in functionalmlds_results:
            ...
            if _can_reuse_stage(
                ...
                stage_id="project_materialization",
                validation_path=validation_path,
                output_paths=[project_json, room_plan, agents, trace_map, validation_path],
                input_paths=[source_mlds, agent_roles_path, placements_path, knowledge_path, functionalmlds_path],
            ):
