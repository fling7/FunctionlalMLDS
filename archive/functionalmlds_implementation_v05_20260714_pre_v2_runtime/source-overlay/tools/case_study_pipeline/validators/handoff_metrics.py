...
def compute_handoff_metrics(
    *,
    ...
    observed_handoff_events = [
        event for event in runtime_events if event.get("event_type") in {"backend_handoff_completed", "unity_handoff_arrived"}
    ]
...
def run_handoff_metrics_for_case(case_dir: Path, *, config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    update_manifest(
        ...
        input_paths=[agent_roles_path, handoff_path, runtime_log_path, *test_paths],
