...
class KnowledgeBase:
    ...
    def search(self, query: str, tags: Sequence[str], k: int = 4) -> List[Dict]:
        ...
        out: List[Dict] = []
        for score, c in top:
            out.append(self._chunk_to_dict(c, score=score))
        return out

    def snippets(self, tags: Sequence[str], k: int = 4) -> List[Dict]:
        tag_set = set([t.strip() for t in tags if t.strip()])
        candidates = [c for c in self._chunks if (not tag_set or c.tag in tag_set)]
        return [self._chunk_to_dict(c, score=0.0) for c in candidates[: max(0, k)]]

    @staticmethod
    def _chunk_to_dict(c: KnowledgeChunk, score: float = 0.0) -> Dict:
        return {
            "tag": c.tag,
            "file": c.file_path,
