...
public class QuickAgentManager : MonoBehaviour
{
    ...
    [Header("FunctionalMLDS Runtime Logging")]
    public bool enableUnityRuntimeLogging = false;
    public FunctionalMLDSUnityRuntimeLogger runtimeLogger;
    ...
    private string _pendingHandoffFromId = "";
    ...
    private IEnumerator SendChat(string message)
    {
        ...
        using (var req = new UnityWebRequest(url, "POST"))
        {
            ...
            LogUnityChatReceived(resp, isHandoff);
    ...
    private void SetActiveAgentId(string id, bool updateStatus = false)
    {
        ...
        if (updateStatus)
        {
            ...
            LogUnityAgentSelected(activeAgentId, "manual");
        }
        UpdateAgentHighlights();
    }

    private void EnsureUnityRuntimeLogger()
    {
        if (!enableUnityRuntimeLogging)
            return;

        if (runtimeLogger == null)
            runtimeLogger = GetComponent<FunctionalMLDSUnityRuntimeLogger>();
        if (runtimeLogger == null)
            runtimeLogger = gameObject.AddComponent<FunctionalMLDSUnityRuntimeLogger>();
        runtimeLogger.enableLogging = true;
    }

    private bool IsUnityRuntimeLoggingReady()
    {
        if (!enableUnityRuntimeLogging)
            return false;
        EnsureUnityRuntimeLogger();
        return runtimeLogger != null && runtimeLogger.enableLogging;
    }

    private string CurrentRuntimeCaseId()
    {
        if (useProjectSelection && !string.IsNullOrWhiteSpace(selectedProjectId))
            return selectedProjectId;
        return "unity_manual_room";
    }

    private void LogUnitySetupCompleted(SetupResponse resp)
    {
        if (!IsUnityRuntimeLoggingReady() || resp == null)
            return;

        runtimeLogger.LogSetupCompleted(
            CurrentRuntimeCaseId(),
            resp.session_id,
            memoryMode,
            lastAgents == null ? 0 : lastAgents.Length);
    }

    private void LogUnityAgentSelected(string agentId, string source)
    {
        if (!IsUnityRuntimeLoggingReady())
            return;
        runtimeLogger.LogAgentSelected(CurrentRuntimeCaseId(), sessionId, agentId, source);
    }

    private void LogUnityChatSent(string message)
    {
        if (!IsUnityRuntimeLoggingReady())
            return;
        runtimeLogger.LogChatSent(CurrentRuntimeCaseId(), sessionId, activeAgentId, string.IsNullOrEmpty(message) ? 0 : message.Length);
    }

    private void LogUnityChatReceived(ChatResponse resp, bool handoff)
    {
        if (!IsUnityRuntimeLoggingReady() || resp == null)
            return;
        var eventCount = resp.events == null ? 0 : resp.events.Length;
        runtimeLogger.LogChatReceived(CurrentRuntimeCaseId(), resp.session_id, resp.active_agent_id, eventCount, handoff);
    }

    private void LogUnityHandoffArrived(string fromAgentId, string toAgentId, ChatEvent[] events)
    {
        if (!IsUnityRuntimeLoggingReady())
            return;
        runtimeLogger.LogHandoffArrived(
            CurrentRuntimeCaseId(),
            sessionId,
            fromAgentId,
            toAgentId,
    ...
    private void TriggerPendingHandoffArrival()
    {
        ...
        var fromAgentId = _pendingHandoffFromId;
        var events  = _pendingHandoffEvents;

        ClearPendingHandoff();
        SetActiveAgentId(agentId);
        LogUnityHandoffArrived(fromAgentId, agentId, events);
    ...
    private void SetPendingHandoff(ChatResponse resp, ChatEvent[] targetEvents)
    {
        ...
        _pendingHandoffFromId = ResolveAgentId(resp.handoff?.from);
