...
| Asset | Ja, als semantische Rolle der Entity | Die Kaffeemaschine ist ein virtuelles Szenenobjekt und kann im aktuellen v0.5-Kern als `Entity.kind = asset` typisiert werden. Feingranulare Typen wie `interactionObject` gehoeren nicht in die Kern-Enumeration. |
...
| Semantische Kategorie | Bedienbares Asset/Interaktionsobjekt | Grob als `Entity.kind = asset` modellierbar; Bedienpunkte, Affordances und Manipulationsregeln bleiben optionales Interaktionsobjekt-Modul. |
