\section{Detailed Contract and Evaluation Tables}

\subsection{Running-example identifiers}

Table~\ref{tab:running-example} spells out the exact objects used for the
\path{brand_panel3} walkthrough.  Display labels are shortened in the prose,
while execution uses these stable identifiers.

\begin{table*}[h]
  \caption{Exact contract objects for the Steinpilz
  \texttt{brand\_panel3} interaction.}
  ...
  \begin{tabularx}{\textwidth}{@{}p{28mm}Y Y@{}}
    ...
    \midrule
    Target
      & \path{brand_panel3} $\rightarrow$
        \path{ENT-ASSET-BRAND_PANEL3}; group
        \path{ENT-GROUP-BRANDING}; zone
        \path{ENT-ZONE-ENTRANCE_ZONE}
      & Collider observation and trusted model referent \\
    Use case and scenario
      & \path{UC-STEINPILZ_BRAND_ROOM-INTERACT-BRAND_PANEL3};
        \path{SC-STEINPILZ_BRAND_ROOM-INTERACT-BRAND_PANEL3-MAIN}
      & User-facing interaction and executable flow \\
    Answer step
      & \path{STEP-STEINPILZ_BRAND_ROOM-INTERACT-BRAND_PANEL3-ANSWER}
      & Scenario occurrence requiring one capability use \\
    Capability use
      & \path{CU-STEINPILZ_BRAND_ROOM-INTERACT-BRAND_PANEL3-ANSWER-ROOM-GROUNDED-QUESTION};
        provider \path{ENT-AGENT-RECEPTION_AGENT}
    ...
    Assertion and case
      & \path{SA-STEINPILZ_BRAND_ROOM-ANSWER-GROUNDED};
        \path{VC-STEINPILZ_BRAND_ROOM-INTERACT-BRAND_PANEL3}
