\section{Supporting Contract and Runtime Details}
\label{app:contract-details}
...
Section~\ref{sec:implementation} introduces the model layers used at runtime.
Table~\ref{tab:functionalmlds-relations} records the selected elements and
references needed to construct the complete \contractpath{} paths evaluated in
RQ1; it is a compact reader map rather than a full serialization schema.
...
Table~\ref{tab:enforcement-layers} expands the staged checks summarized in
Sections~\ref{sec:contract} and~\ref{sec:implementation}.  The backend remains
authoritative even when the client performs an earlier equivalent check.
...
\section{Per-Case Evaluation Results}
\label{app:per-case-results}

Tables~\ref{tab:per-case-chains} and~\ref{tab:per-case-routing} provide the
per-case denominators behind the aggregate RQ1 results.  The routing table
separates one-turn local/direct admission from transitive graph reachability.
...
The reproducible technical benchmark is rooted at
...
\texttt{research/iui2027/artifact}.  Study U analysis code is under
\texttt{research/iui2027/paper\_userstudy/analysis}; the dated questionnaire
export and study materials belong to the anonymous supplementary package.
Backend runtime code is under
...
remains in \texttt{research/iui2027/paper}; this user-study manuscript is kept
separate in \texttt{research/iui2027/paper\_userstudy}.
