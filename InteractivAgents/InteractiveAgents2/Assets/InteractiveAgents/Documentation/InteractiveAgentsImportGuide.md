...
- `/health` and `/version` for the backend/API version shown in the wizard
- `/setup`
- `/chat`
- `/tts`
- `/stt`
- `/projects/arrow/placement` for validated position/forward updates from the map
- `/projects/...`

The imported MLDSI Wizard shows its own version separately from the backend and
API versions. In the placement preview every agent is labelled as `Name [id]`.
Drag a marker to move it on the X/Z floor plane; select it and drag the yellow
forward handle to rotate it. Apply validates and persists the edit in the
backend, while reset restores the last accepted placement.
