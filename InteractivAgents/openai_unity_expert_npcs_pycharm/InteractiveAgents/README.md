...
Teste mit:
- `GET /health`
- `GET /version` (liefert dieselbe zentrale Backend-Version wie `/health`)
- `POST /setup` ohne Payload nutzt die beim Start gewählten Pfade (oder die Beispiel-Daten).

`GET /health` und `GET /version` liefern derzeit beispielsweise:

```json
{"status": "ok", "backend_version": "1.0.0", "api_version": "1.0"}
```

Die Backend-Version ist unabhängig von der Unity-/Wizard- und der FunctionalMLDS-
Metamodell-Version.

Der MLDSI-Wizard kann bereits analysierte Agentenplatzierungen mit
`POST /projects/arrow/placement` aktualisieren. Der Request enthält die
`session_id` und für jeden Agenten exakt einen Eintrag in `agent_placements`
mit endlicher `position` sowie einem planaren, normalisierten `forward`-Vektor.
Die Position muss auf der Bodenebene liegen (`position.y = 0`).
Ungültige Updates werden strukturiert zurückgegeben und verändern weder Draft
noch FunctionalMLDS-Artefakte.
...
- `QuickAgentManager.cs`: Unity-Client für GUI, Setup, Spawning, Chat, Voice Input und TTS
- `FunctionalMldsV2QuickAgentBridge.cs` und `FunctionalMldsV2/`: nativer V2-Loader,
  exakte Capability-Kette, Assertions, V&V und Runtime-Logging; diese Dateien sind
  Pflichtabhängigkeiten des aktuellen `QuickAgentManager.cs`
- `ArrowProjectWizard.cs` und `InteractiveAgentsVersion.cs`: MLDSI-Wizard v1.0.0
  mit Backend-/API-Versionsanzeige, beschrifteter Placement-Karte sowie
  validiertem Verschieben und Drehen der Agenten
...
3. Im Unity Package Manager `com.unity.nuget.newtonsoft-json` in Version `3.2.2`
   eintragen beziehungsweise installieren.
4. `unity_scripts/QuickAgentManager.cs`,
   `unity_scripts/FunctionalMldsV2QuickAgentBridge.cs` und den vollständigen Ordner
   `unity_scripts/FunctionalMldsV2/` nach `Assets/Scripting/` kopieren. Die Dateien
   dürfen nicht einzeln oder aus unterschiedlichen Versionen übernommen werden.
5. Für WebGL/WebXR zusätzlich `unity_scripts/Plugins` nach `Assets/Plugins` kopieren.
6. `QuickAgentManager` auf ein leeres GameObject ziehen (z.B. `AgentManager`).
7. **Play** drücken.

Für den Editor-Wizard zusätzlich `unity_scripts/ArrowProjectWizard.cs` und
`unity_scripts/InteractiveAgentsVersion.cs` gemeinsam nach `Assets/Scripting/`
kopieren. Danach steht er unter `Tools > MLDSI Project Wizard` bereit. Beide
Dateien müssen aus demselben Stand stammen, damit die angezeigte Wizard-Version
und der Placement-API-Vertrag zusammenpassen.
